...
namespace FindObjectsByIdFromCollection
{
	class Program
	{
		...
		{
			var collectionOfIds = new List<int> { 5, 6, 7, 32};
			...
			var resultsAny = elements.Where(p => collectionOfIds.Any(p2 => p2 == p.Id)); // те, которые есть 5, 6,7




			var results = elements.Where(p => collectionOfIds.All(p2 => p2 != p.Id)).ToList(); // те, которых нет выведутся 1, 2, 3, 4 id  c объектами.
